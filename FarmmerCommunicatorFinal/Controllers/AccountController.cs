    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                var user = new ApplicationUser { UserName = model.Mobile, Address=model.Address, Mobile=model.Mobile, Name=model.Name, Email=model.Mobile+"@communicator.com"};
                var result = await UserManager.CreateAsync(user, model.Password);
                if (result.Succeeded)
                {
                    
                    if (model.UserRoles == "Farmer")
                    {
                        await UserManager.AddToRoleAsync(user.Id, "Farmer");
                    }
                    else
                    {
